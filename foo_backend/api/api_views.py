...
def get_user_list(request):
    try:
        ...
        serialized = UserCustomSerializer(qs,many=True,context={"type":""})
...
def get_friends_list(request):
    try:
        ...
        qs = friends.filter(Q(username_alias__icontains=param)
                                 | Q(f_name__icontains=param))
        # .filter(l_name__icontains=param)
        serialized = UserCustomSerializer(qs,many=True,context={"type":"chat"})
        print(request.query_params)
        # print(data)
        return Response(status=200, data=serialized.data)
    except Exception as e:
...
def upload_chat_media(request):
    try:
        ...
        async_to_sync(channel.group_send)(str(from_user.uprn),msg)
        ...
        if other_user.profile.online:
            async_to_sync(channel.group_send)(str(other_user.uprn),{'type':'chat_message','message':message})
        else:
...
def upload_chat_image_reply(request):
    try:
        ...
        other_user = User.objects.get(username=other_user_username)
        if other_user.profile.online:
            async_to_sync(channel.group_send)(str(other_user.uprn),{'type':'chat_reply_message','message':message,'msg_type':'reply_img'})
        else:
            send_anonymous_notif.delay(other_user.id)
...
def upload_chat_audio_reply(request):
    try:
        ...
        if other_user.profile.online:
            async_to_sync(channel.group_send)(str(other_user.uprn),{'type':'chat_reply_message','message':message,'msg_type':'reply_aud'})
        else:
            send_anonymous_notif.delay(other_user.id)
...
def people_you_may_know(request):
    try:
        ...
        serialized = UserCustomSerializer(req_qs,many=True,context={"type":""})
        return Response(status=200, data=serialized.data)
    except Exception as e:
        print(e)
        return Response(status=400)
...
def friends_list(request):
    try:
        ...
        serialized = UserCustomSerializer(friends_qs, many=True,context={"type":""})
        return Response(status=200, data=serialized.data)
    except Exception as e:
        return Response(status=400)

@api_view(['GET'])
def liked_users_list(request):
    try:
        id = int(request.query_params['post_id'])
        post = Post.objects.get(id=id)
        likes = post.likes.all()
        serialized = UserCustomSerializer(likes, many=True,context={"type":""})
        return Response(status=200, data=serialized.data)
    except Exception as e:
        print(e)
        return Response(status=400)
